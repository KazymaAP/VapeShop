...
const nextConfig = {
    ...
    return [
      {
        ...
        headers: [
          ...
          },
          {
            ...
            value:
              "default-src 'self'; script-src 'self' https://telegram.org; style-src 'self'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https://api.telegram.org https://*.supabase.co; frame-ancestors 'self'; form-action 'self'",
          },
        ],
      },
    ];
  ...
  // All 10 CRITICAL issues are fixed. ESLint warnings (dependency arrays) will be fixed in MEDIUM priority.
  eslint: {
    ignoreDuringBuilds: false, // Detect ESLint errors
  },

  typescript: {
    tsc: false,
    ignoreBuildErrors: false, // Detect TypeScript errors
  },
};
