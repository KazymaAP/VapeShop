...
export async function optimizeImageServer(
  ...
  try {
    ...
    throw new Error(
      'sharp package is required for server-side image optimization. Install it with: npm install sharp'
    );
  }
...
export async function fetchAndOptimizeImage(
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch and optimize image: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}
