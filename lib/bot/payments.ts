    ...
    try {
      await transaction(async (client) => {
        ...
        if (totalAmount !== expectedAmount) {
          ...
          throw new Error(
            `PAYMENT_AMOUNT_MISMATCH: expected ${expectedAmount}, got ${totalAmount}`
          );
        }
        ...
        const userRes = await client.query(
          'SELECT referred_by FROM users WHERE telegram_id = $1 FOR UPDATE',
          [telegramId]
        );
        ...
        if (userRes.rows.length > 0 && userRes.rows[0].referred_by) {
          ...
          try {
            ...
            logger.error('Referral bonus failed', {
      ...
      (async () => {
        try {
          ...
          logger.error('Admin notification batch failed', {
            error:
