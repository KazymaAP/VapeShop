...
export async function getTelegramIdFromRequest(req: NextApiRequest): Promise<number | null> {
  try {
    ...
    // Development fallback для локального тестирования (НЕ БЕЗОПАСНО для production!)
    if (process.env.NODE_ENV === 'development') {
      const devId = req.headers['x-telegram-id'];
      if (devId) {
        const telegramId = parseInt(String(devId), 10);
        if (!isNaN(telegramId)) {
          logger.warn('[DEV] Using X-Telegram-Id header (NOT SECURE - Development only)');
          return telegramId;
        }
      }
    }
