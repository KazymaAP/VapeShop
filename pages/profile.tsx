...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to load user profile (HTTP ${res.status}): GET /api/users/profile?telegram_id=${user?.id}`
      );
    }
  ...
  const fetchOrders = async () => {
    ...
    if (!res.ok) {
      throw new Error(
        `Failed to load user orders (HTTP ${res.status}): GET /api/orders?telegram_id=${user?.id}`
      );
    }
    const data = await res.json();
