    ...
    try {
      ...
      await transaction(async (client) => {
        if (action === 'clear_abandoned_carts') {
          // Удаляем только заказы старше 7 дней (не используемые)
          await client.query("DELETE FROM carts WHERE updated_at < NOW() - INTERVAL '7 days'", []);
        } else if (action === 'clear_admin_logs') {
          // Удаляем только логи старше 90 дней
          await client.query(
