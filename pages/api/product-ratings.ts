...
async function handlePost(req: NextApiRequest, res: NextApiResponse<ApiResponseType>) {
  try {
    ...
    if (!productId || !rating) {
      return res
        .status(400)
        .json({ success: false, error: 'Missing required fields', timestamp: Date.now() });
    }

    if (rating < 1 || rating > 5) {
      return res
        .status(400)
        .json({ success: false, error: 'Rating must be between 1 and 5', timestamp: Date.now() });
    }
    ...
    const orderCheck = await query(
      `SELECT o.id FROM orders o
       ...
       WHERE oi.product_id = $1 AND o.user_telegram_id = $2 AND o.status IN ('done', 'delivered')
       LIMIT 1`,
    ...
    if (orderCheck.rows.length === 0) {
      return res.status(403).json({
        success: false,
        error: 'You can only rate products you purchased',
        timestamp: Date.now(),
      });
    }
...
async function handleGet(req: NextApiRequest, res: NextApiResponse<ApiResponseType>) {
  try {
    ...
    if (!productId) {
      return res
        .status(400)
        .json({ success: false, error: 'productId is required', timestamp: Date.now() });
    }
