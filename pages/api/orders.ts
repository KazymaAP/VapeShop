...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const productsRes = await query(
      'SELECT id, stock, price FROM products WHERE id = ANY($1::int[]) AND is_active = true',
      [productIds]
    );
    ...
    const order = await transaction(async (client) => {
      ...
      const lockedProducts = await client.query(
        'SELECT id, stock, price FROM products WHERE id = ANY($1::int[]) AND is_active = true FOR UPDATE',
        [productIds]
      );
