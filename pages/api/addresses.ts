  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const result = await transaction(async (client) => {
        if (is_default) {
          await client.query(
            'UPDATE addresses SET is_default = false WHERE user_telegram_id = $1',
            [currentTelegramId]
          );
        }
    ...
    try {
      ...
      await transaction(async (client) => {
        const addrRes = await client.query(
          'SELECT user_telegram_id FROM addresses WHERE id = $1 FOR UPDATE',
          [id]
        );
        if (!addrRes.rows[0]) {
    ...
    try {
      ...
      await transaction(async (client) => {
        ...
        await client.query('UPDATE addresses SET is_default = $1 WHERE id = $2', [
          is_default || false,
          id,
        ]);
      });
