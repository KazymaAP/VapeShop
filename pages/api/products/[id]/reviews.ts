...
async function handleGetReviews(
  ...
  try {
    ...
    const countResult = await query('SELECT COUNT(*) as cnt FROM product_reviews WHERE product_id = $1', [
      productId,
    ]);
  ...
  try {
    ...
    if (rating) {
      try {
        ...
        logger.warn('Failed to update product rating:', { error: err instanceof Error ? err.message : String(err) });
      }
    }
...
async function handleDeleteReview(
  ...
  try {
    ...
    try {
      ...
      logger.warn('Failed to update product rating after review deletion:', { error: err instanceof Error ? err.message : String(err) });
    }
