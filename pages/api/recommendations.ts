...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiResponseType>) {
  ...
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ success: false, error: 'userId required', timestamp: Date.now() });
    }
    ...
    if (userCategories.rows.length === 0) {
      ...
      const popular = await query(
        `SELECT id, name, price, images[1] as image_url, category_id, rating
         FROM products
         WHERE is_active = true
         ORDER BY rating DESC, views DESC, created_at DESC
         LIMIT $1`,
        [limit]
      );
    ...
    const categories = userCategories.rows.map((r) => r.category_id);

    // Рекомендуем товары из похожих категорий
    let query_str = `
      SELECT id, name, price, images[1] as image_url, category_id, rating, 0 as rating_count
      FROM products
      WHERE is_active = true
